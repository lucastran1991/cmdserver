...
# Main script to update the config file
  ...
  sed -i "79s/LOCAL/REMOTE/" $currentSource/server/sff.auto.config.cdm
  sed -i "80s/LOCAL/REMOTE/" $currentSource/server/sff.auto.config.cdm
  sed -i "81s/LOCAL/REMOTE/" $currentSource/server/sff.auto.config.cdm
  sed -i "82s/LOCAL/REMOTE/" $currentSource/server/sff.auto.config.cdm
  sed -i "83s/LOCAL/REMOTE/" $currentSource/server/sff.auto.config.cdm
...
restart_server(){
  ...
  nohup java @java-options.txt -jar tql.engine2.4.jar &> nohup.out &
  sleep 10
  echo "Backend server started successfully"
  sleep 15  
  nohup python /data2/Atomiton/WaterTRN/QA/pyastackcore/pyastackcore/co_engine.py > output.log &
}
...
grep_eror_log() {
  cd $currentSource/server/
  tail -f "$currentSource/server/nohup.out" | grep --line-buffered "error"
}
...
echo "8. View engine.log Log file."
echo "9. Clear The Cached."
echo "10. View Nohup.out Log file"
echo "11. View Co_engine Log file"
echo "12. Deploy the latest UI Veolia Plugin Commit."
echo "13. Kill all Engine."
echo "14. View ERROR Logs only - NOHUP.out"

read -p "Enter your choice [1, 2, 3, ... , or 14]: " choice
case $choice in
  1) pull_be_source;;
